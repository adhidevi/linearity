#define AnalyzeMps_rNew_cxx
#include "AnalyzeMps_rNew.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <fstream>

void AnalyzeMps_rNew::Loop(Int_t run,Int_t pmt,Double_t preAmp,Int_t HV,Double_t LL,Double_t Iout,Int_t FF,Double_t Ped_X1,Double_t Ped_X2,Double_t asym_X1,Double_t asym_X2,Double_t chsum,Double_t asym1_X1,Double_t asym1_X2,Double_t asym8_X1,Double_t asym8_X2,Double_t asymF_X1,Double_t asymF_X2,Double_t asymP_X1,Double_t asymP_X2,Double_t asym_Y1,Double_t asym_Y2)
{
//   In a ROOT session, you can do:
//      root> .L AnalyzeMps_rNew.C
//      root> AnalyzeMps_rNew t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   if (fChain == 0) return;
     ...
     TH1F *h1_ped = new TH1F("h1_ped", "", 100, Ped_X1, Ped_X2);
     TH1F *h1_ped_even = new TH1F("h1_ped_even", "", 100, Ped_X1, Ped_X2);
     TH1F *h1_ped_odd = new TH1F("h1_ped_odd", "", 100, Ped_X1, Ped_X2);
...
//        havg[f]->SetMaximum(8);
     ...
     Double_t EvenSum[octsize], OddSum[octsize];
     Int_t EventNum[arraysize];
     Double_t SumArray[octsize/2];
     Double_t hw_sum_raw[arraysize];
     Double_t ChSum_even, ChSum_odd, raw_sum0[octsize], raw_sum1[octsize], raw_sum2[octsize], raw_sum3[octsize], raw_sum4[octsize], raw_sum5[octsize], raw_sum6[octsize], raw_sum7[octsize];
     Int_t Q0 = 0;
     Int_t Q1 = 0;
     Int_t Q2 = 0;
     Int_t Q3 = 0;
...
//     ofstream outfile2("sum.txt");
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     if (ientry < 0) break;
          ...
          EventNum[jentry] = CodaEventNumber;
          hw_sum_raw[jentry] = qwk_mod0ch6_hw_sum_raw;
	  h1_raw->Fill(EventNum[jentry], hw_sum_raw[jentry]);
	  outfile << EventNum[jentry] << "	" << hw_sum_raw[jentry] << endl;
	...
	if(hw_sum_raw[jentry]<chsum){
	  h1_ped->Fill(hw_sum_raw[jentry]);
	if(EventNum[jentry]/2.0-int(EventNum[jentry]/2) ==0){    
          h1_ped_even->Fill(hw_sum_raw[jentry]);
        }else{
	  h1_ped_odd->Fill(hw_sum_raw[jentry]);
	}
	}
	}
...
//	outfile << pairNumber << "	" << group << "	" << group_data_count << "	" << Asymmetry << endl;
	  ...
	  Navg[group-1] += sum[i]/8/280/samples/blocks;
       }
       }
...
//	  outfile2.close();
    ...
    char runLabel[256];
    sprintf(runLabel,"run %4d, PreAmp @ %3.2f M#Omega LED flash @ %4d Hz:", run, preAmp,FF);
    TPaveLabel *pt_rl = new TPaveLabel(0.25,0.85,0.58,0.89,runLabel,"NDC");
    ...
    gPad->SetGridy(1);
    h1_asym_my->Draw();
    c->SaveAs(Form("r%dplotsMps.png",run));
    ...
    pt1->SetFillColor(0);
    pt1->Draw();
    sprintf(labelF[0],"1 #pm 0.1%% ND filter");
    TPaveLabel *ptF1 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[0],"NDC");
    ptF1->SetBorderSize(0);
    ptF1->SetTextColor(kMagenta);
    ptF1->SetTextSize(1.0);
    ptF1->SetFillColor(0);
    ptF1->Draw();
    sprintf(labelRun,"run %4d, %5d V, I_pe #approx %2.2f nA, %3d Hz", run, HV, LL, FF);
    TPaveLabel *ptR1 = new TPaveLabel(0.12,0.91, 0.68,0.99,labelRun,"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+3]->Draw();
    sprintf(label[1],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+3], Navg[8*i+3]/Navg[8*i+0]*100);
    TPaveLabel *pt2 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[1],"NDC");
    pt2->SetBorderSize(0);
    pt2->SetTextColor(kBlack);
    pt2->SetTextSize(1.0);
    pt2->SetFillColor(0);
    pt2->Draw();
    sprintf(labelF[1],"50 #pm 5%% ND filter");
    TPaveLabel *ptF2 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[1],"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+4]->Draw();
    sprintf(label[2],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+4], Navg[8*i+4]/Navg[8*i+0]*100);
    TPaveLabel *pt3 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[2],"NDC");
    pt3->SetBorderSize(0);
    pt3->SetTextColor(kBlack);
    pt3->SetTextSize(1.0);
    pt3->SetFillColor(0);
    pt3->Draw();
    sprintf(labelF[2],"63 #pm 5%% ND filter");
    TPaveLabel *ptF3 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[2],"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+5]->Draw();
    sprintf(label[3],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+5], Navg[8*i+5]/Navg[8*i+0]*100);
    TPaveLabel *pt4 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[3],"NDC");
    pt4->SetBorderSize(0);
    pt4->SetTextColor(kBlack);
    pt4->SetTextSize(1.0);
    pt4->SetFillColor(0);
    pt4->Draw();
    sprintf(labelF[3],"25 #pm 2.5%% ND filter");
    TPaveLabel *ptF4 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[3],"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+6]->Draw();
    sprintf(label[4],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+6], Navg[8*i+6]/Navg[8*i+0]*100);
    TPaveLabel *pt5 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[4],"NDC");
    pt5->SetBorderSize(0);
    pt5->SetTextColor(kBlack);
    pt5->SetTextSize(1.0);
    pt5->SetFillColor(0);
    pt5->Draw();
    sprintf(labelF[4],"79 #pm 5%% ND filter");
    TPaveLabel *ptF5 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[4],"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+7]->Draw();
    sprintf(label[5],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+7], Navg[8*i+7]/Navg[8*i+0]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[5],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[5],"40 #pm 4%% ND filter");
    TPaveLabel *ptF6 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[5],"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+0]->Draw();
    sprintf(label[6],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+0], Navg[8*i+0]/Navg[8*i+0]*100);
    TPaveLabel *pt7 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[6],"NDC");
    pt7->SetBorderSize(0);
    pt7->SetTextColor(kBlack);
    pt7->SetTextSize(1.0);
    pt7->SetFillColor(0);
    pt7->Draw();
    sprintf(labelF[6],"100%% (No filter)");
    TPaveLabel *ptF7 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[6],"NDC");
    ...
    gPad->SetGridy(1);
    h[8*i+1]->Draw();
    sprintf(label[7],"N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+1], Navg[8*i+1]/Navg[8*i+0]*100);
    TPaveLabel *pt8 = new TPaveLabel(0.15,0.84, 0.55,0.89,label[7],"NDC");
    pt8->SetBorderSize(0);
    pt8->SetTextColor(kBlack);
    pt8->SetTextSize(1.0);
    pt8->SetFillColor(0);
    pt8->Draw();
    sprintf(labelF[7],"10 #pm 1%% ND filter");
    TPaveLabel *ptF8 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelF[7],"NDC");
    ...
    sprintf(labelFPed,"Shutter Closed");
    TPaveLabel *ptF9 = new TPaveLabel(0.15,0.75, 0.40,0.81,labelFPed,"NDC");
    ptF9->SetBorderSize(0);
    ptF9->SetTextColor(kMagenta);
    ptF9->SetTextSize(1.0);
    ptF9->SetFillColor(0);
    ptF9->Draw();
    c21->SaveAs(Form("r%d_asyms_auto_20cyc_c%d.png",run,i));
	}
   ...
   for(int i=0;i<filters;i++){
    ...
    dummyError[0] = 0.0;
    char label[filters][256], labelF[filters][256], labelRun[256];
    sprintf(label[0],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[0], Navgmean[0]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[0],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[0],"1 #pm 0.1% ND filter");
    TPaveLabel *ptF1 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[0],"NDC");
    ptF1->SetBorderSize(0);
    ptF1->SetTextColor(kMagenta);
    ptF1->SetTextSize(1.0);
    ptF1->SetFillColor(0);
    ptF1->Draw();
    sprintf(labelRun,"run %4d, %5d V, I_pe #approx %2.2f nA, %3d Hz", run, HV, LL, FF);
    TPaveLabel *ptR1 = new TPaveLabel(0.32,0.94, 0.65,0.99,labelRun,"NDC");
    ptR1->SetBorderSize(0);
    ptR1->SetTextColor(kRed);
    ptR1->SetTextSize(0.80);
    ptR1->SetFillColor(0);
    ...
    dummyError[1] = 0.0;
    sprintf(label[1],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[1], Navgmean[1]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[1],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[1],"50 #pm 5%% ND filter");
    TPaveLabel *ptF2 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[1],"NDC");
    ptF2->SetBorderSize(0);
    ptF2->SetTextColor(kMagenta);
    ptF2->SetTextSize(1.0);
    ptF2->SetFillColor(0);
    ...
    dummyError[2] = 0.0;
    sprintf(label[2],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[2], Navgmean[2]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[2],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[2],"63 #pm 5%% ND filter");
    TPaveLabel *ptF3 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[2],"NDC");
    ptF3->SetBorderSize(0);
    ptF3->SetTextColor(kMagenta);
    ptF3->SetTextSize(1.0);
    ptF3->SetFillColor(0);
    ...
    dummyError[3] = 0.0;
    sprintf(label[3],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[3], Navgmean[3]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[3],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[3],"25 #pm 2.5%% ND filter");
    TPaveLabel *ptF4 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[3],"NDC");
    ptF4->SetBorderSize(0);
    ptF4->SetTextColor(kMagenta);
    ptF4->SetTextSize(1.0);
    ptF4->SetFillColor(0);
    ...
    dummyError[4] = 0.0;
    sprintf(label[4],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[4], Navgmean[4]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[4],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[4],"79 #pm 5%% ND filter");
    TPaveLabel *ptF5 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[4],"NDC");
    ptF5->SetBorderSize(0);
    ptF5->SetTextColor(kMagenta);
    ptF5->SetTextSize(1.0);
    ptF5->SetFillColor(0);
    ...
    dummyError[5] = 0.0;
    sprintf(label[5],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[5], Navgmean[5]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[5],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[5],"40 #pm 4%% ND filter");
    TPaveLabel *ptF6 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[5],"NDC");
    ptF6->SetBorderSize(0);
    ptF6->SetTextColor(kMagenta);
    ptF6->SetTextSize(1.0);
    ptF6->SetFillColor(0);
    ...
    dummyError[6] = 0.0;
    sprintf(label[6],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[6], Navgmean[6]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[6],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[6],"100%% (No filter)");
    TPaveLabel *ptF7 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[6],"NDC");
    ptF7->SetBorderSize(0);
    ptF7->SetTextColor(kMagenta);
    ptF7->SetTextSize(1.0);
    ptF7->SetFillColor(0);
    ...
    dummyError[7] = 0.0;
    sprintf(label[7],"N_avg = %4.3e (%4.1f %%)" ,Navgmean[7], Navgmean[7]/Navgmean[6]*100);
    TPaveLabel *pt6 = new TPaveLabel(0.25,0.85, 0.55,0.89,label[7],"NDC");
    pt6->SetBorderSize(0);
    pt6->SetTextColor(kBlack);
    pt6->SetTextSize(1.0);
    pt6->SetFillColor(0);
    pt6->Draw();
    sprintf(labelF[7],"10 #pm 1%% ND filter");
    TPaveLabel *ptF8 = new TPaveLabel(0.25,0.80, 0.38,0.84,labelF[7],"NDC");
    ptF8->SetBorderSize(0);
    ptF8->SetTextColor(kMagenta);
    ptF8->SetTextSize(1.0);
    ptF8->SetFillColor(0);
    ptF8->Draw();
    c_avg->SaveAs(Form("r%d_asyms_auto_20cyc_avg.png",run));
   ...
   leg->Draw();
   c3->SaveAs(Form("r%d_asymTrends_auto_20cyc.png",run));   
   ...
   pt_cal->Draw();
   sprintf(fitLabel,"run %4d, %3d Hz, %3.2f M#Omega, I_{anode} = %4.2f #mu A", run, FF, preAmp, Iout);
   TPaveLabel *pt_rlcal = new TPaveLabel(0.20,0.84,0.50,0.89,fitLabel,"NDC");
   pt_rlcal->SetBorderSize(0);
   pt_rlcal->SetTextColor(kBlack);
   pt_rlcal->SetTextSize(0.75);
   pt_rlcal->SetFillColor(0);
   pt_rlcal->Draw();
   char labelB[256], labelBB[256];
   sprintf(labelB,"#beta*N_{avg} @ %4.3f = %5.3f #pm %4.3f %% non-lin",Navgmean_Cal[6], f3par[1]/f3par[0]*Navgmean_Cal[6]*100, fabs(sqrt(pow(Error20/f3par[0],2) + pow(Error21/f3par[1],2))*f3par[1]/f3par[0]*Navgmean_Cal[6]*100)*scale);
   TPaveLabel *ptB = new TPaveLabel(0.11,0.78, 0.53,0.82,labelB,"NDC");
   ptB->SetBorderSize(0);
   ptB->SetTextColor(kMagenta);
   ptB->SetTextSize(0.80);
   ptB->SetFillColor(0);
   ptB->Draw();
   sprintf(labelBB,"#chi^{2}                                                %4.4f",f3->GetChisquare()/f3->GetNDF());
   TPaveLabel *ptBB = new TPaveLabel(0.63,0.888, 0.97,0.928,labelBB,"NDC");
   ...
   leg->SetFillStyle(0);
   leg->AddEntry(gr1,"data","pe");
   leg->AddEntry(f3,"fit: A_{LED} = A_{true}(1+#beta*N_{avg})","l");
   leg->Draw();
   c3Cal->SaveAs(Form("r%d_asymTrends_auto_20cyc_Cal.png",run));
   }
