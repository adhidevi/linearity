...
void AnalyzeMps(Int_t run,Int_t pmt,Double_t preAmp,Int_t HV,Double_t LL,Double_t Iout,Int_t FF,Double_t asym_X1,Double_t asym_X2,Int_t chsum,Double_t asym1_X1,Double_t asym1_X2,Double_t asym8_X1,Double_t asym8_X2,Double_t asymF_X1,Double_t asymF_X2,Double_t asymP_X1,Double_t asymP_X2,Double_t asym_Y1,Double_t asym_Y2){
...
//     double max_adc_axis = 262000000;//for 120Hz flipping
...
//        havg[f]->SetMaximum(8);
     ...
     Double_t* SumArray = new Double_t[nentries];
     Double_t* EventNumArray = new Double_t[nentries];
     ofstream outfile("diff.txt");
     tree->Draw("qwk_mod0ch4.hw_sum_raw:CodaEventNumber","","goff");

   for (Long64_t jentry=10; jentry<nentries;jentry++) {
	  EventNumArray[jentry] = *(tree->GetV2()+jentry);
	  SumArray[jentry] = *(tree->GetV1()+jentry);
//	  outfile << EventNumArray[jentry] << "	" << SumArray[jentry] << endl;
          h1_raw->Fill(EventNumArray[jentry], SumArray[jentry]);
     if(EventNumArray[jentry]/2.0 - int(EventNumArray[jentry]/2) == 0) {
	  h1_raw_even->Fill(EventNumArray[jentry], SumArray[jentry]);
     } else {
	  h1_raw_odd->Fill(EventNumArray[jentry], SumArray[jentry]);
     }
     }
...
//	outfile << pairNumber << "	" << group << "	" << group_data_count << "	" << Asymmetry << endl;
	  ...
	  Navg[group-1] += pairSum[pairNumber]/2/1100/samples/blocks;
     }
     }
     if(Ped_Event_Num[pairNumber]!=0){
