...
void patternAsym(Int_t run,Int_t pmt,Double_t preAmp,Int_t HV,Double_t LL,Double_t Iout,Int_t FF,Double_t asym_X1,Double_t asym_X2,Int_t chsum_min,Int_t chsum,Double_t asym1_X1,Double_t asym1_X2,Double_t asym8_X1,Double_t asym8_X2,Double_t asymF_X1,Double_t asymF_X2,Double_t asymP_X1,Double_t asymP_X2,Double_t asym_Y1,Double_t asym_Y2){
     int npat = 2;
     TString root_file = Form("./rootfiles/patternCombine_%d_run%d.root",npat,run);
     TChain* tree = new TChain("patternSum");
     tree->Add(root_file);
     if(root_file==NULL){
     ...
     Double_t pattern_sum_0, pattern_sum_1;
     Int_t pattern_number;
     tree->SetBranchAddress("pattern_sum_0",&pattern_sum_0);
     tree->SetBranchAddress("pattern_sum_1",&pattern_sum_1);
     tree->SetBranchAddress("pattern_number",&pattern_number);
     Long64_t nentries = tree->GetEntries();
     cout<<"Total Patterns: "<<nentries<<endl;

     gStyle->SetOptStat(1111);
     
     double max_adc_axis = 26200000;
     ...
     TH2F *h1_asym_my = new TH2F("h1_asym_my", "", nentries/20, 0, nentries, 800, asym_X1, asym_X2);
     ...
     h1_asym_my->SetTitle("Pattern-wise Asymmetry vs. pair No.;Pattern No.;A_{LED}");
     ...
     Int_t samples = 400;
     Int_t blocks = 4;
     Double_t Vperch = 76.29E-6;

     TH1F* h[positions];
     TH1F* hPed;
     char name[20];
     sprintf(name,"hPed_asym");
     hPed = new TH1F(name,"",100, asymP_X1, asymP_X2);
     TH1F* havg[positions];
     Int_t f;
     ...
     for(f=0;f<positions;f++){
      ...
      if((f-2)/8.0 - int((f-2)/8) ==0){
        h[f] = new TH1F(name,"",100, asym1_X1, asym1_X2);
      }else if((f-1)/8.0 - int((f-1)/8) ==0){
        h[f] = new TH1F(name,"",100, asym8_X1, asym8_X2);
      }else{
        h[f] = new TH1F(name,"",100, asymF_X1, asymF_X2);
      }
     ...
     for(f=0;f<filters;f++){
      ...
      if(f==0)
        havg[f] = new TH1F(name,"",100, asym_X1, asym1_X2);
      else
        havg[f] = new TH1F(name,"",100, asymF_X1, asymF_X2);
     ...
     for(Long64_t i=0;i<nentries;i++){
       tree->GetEntry(i);
       cout<<pattern_number<<"\t"<<pattern_sum_0<<"\t"<<pattern_sum_1<<endl;
       h1_raw->Fill(pattern_number,pattern_sum_0);
       h1_raw->Fill(pattern_number,pattern_sum_1);
       h1_raw_even->Fill(pattern_number,pattern_sum_0);
       h1_raw_odd->Fill(pattern_number,pattern_sum_1);
      if(pattern_sum_0<chsum || pattern_sum_1<chsum){
       h1_ped->Fill(pattern_sum_0);
       h1_ped->Fill(pattern_sum_1);
       h1_ped_even->Fill(pattern_sum_0);
       h1_ped_odd->Fill(pattern_sum_1);
      } 
     }

     Double_t pedAve = h1_ped->GetMean();
     Double_t sum, diff, Asymmetry;
     Double_t Filter_Event_Num[nentries], Filter_Asymmetry;
     int group = 0;
     ...
     for(Long64_t i=0;i<nentries;i++){
       tree->GetEntry(i);
       h1_rawCor->Fill(pattern_number,pattern_sum_0-pedAve);
       h1_rawCor->Fill(pattern_number,pattern_sum_1-pedAve);
       h1_rawCor_even->Fill(pattern_number,pattern_sum_0-pedAve);
       h1_rawCor_odd->Fill(pattern_number,pattern_sum_1-pedAve);
       sum = pattern_sum_0+pattern_sum_1-2*pedAve;
       outfile<<i<<"\t"<<sum<<endl;
       diff = pattern_sum_0-pattern_sum_1;
       Asymmetry = fabs(diff/sum);
       if(sum>1.0e6){
         h1_asym_my->Fill(pattern_number,Asymmetry);
         Filter_Event_Num[i] = pattern_number;
         Filter_Asymmetry = Asymmetry;
       }else{
         Filter_Event_Num[i] = 0.0;
         Filter_Asymmetry = 0.0;
         hPed->Fill(diff/(sum+2*pedAve));
       }
       if(Filter_Event_Num[i]!=Filter_Event_Num[i-1]+1&&Filter_Event_Num[i]!=0){
         ...
         group_data_count++;
       if(group_data_count>10&&group_data_count<=140){
         h[group-1]->Fill(Filter_Asymmetry);
         Navg[group-1] += sum/2/130/samples/blocks;
       }
      }
     }
     outfile.close();
    ...
    gPad->SetGridy(1);
    h1_asym_my->Draw();
    c1->SaveAs(Form("test_%d/r%dplotsMps.png",npat,run));
    ...
    gPad->SetGridy(1);
    h[8*i+2]->Draw();
    TPaveLabel *pt1 = new TPaveLabel(0.15,0.84, 0.50,0.89,Form("N_avg = %4.3e (%4.1f %%)" ,Navg[8*i+2], Navg[8*i+2]/Navg[8*i+0]*100),"NDC");
    pt1->SetBorderSize(0);
    ...
    ptF9->SetFillColor(0);
    ptF9->Draw();
    c21->SaveAs(Form("test_%d/r%d_asyms_auto_20cyc_c%d.png",npat,run,i));
   }
   ...
   for(int i=0;i<filters;i++){
    ...
    plF8->SetFillColor(0);
    plF8->Draw();
    c_avg->SaveAs(Form("test_%d/r%d_asyms_auto_20cyc_avg.png",npat,run));
   ...
   ptChi->Draw();
   hr1->SetTitle(Form("Filter wheel scan: pmt#%4d @ %5d V, I_{cath} = %2.2f nA;N_{avg} (adc channels);A_{LED}", pmt, HV, LL));
   ...
   leg->Draw();
   c3->SaveAs(Form("test_%d/r%d_asymTrends_auto_20cyc.png",npat,run));
   ...
   ptBB->SetFillColor(0);
   ptBB->Draw();
   hr2->SetTitle(Form("Filter wheel scan: pmt#%4d @ %5d V, I_{cath} = %2.2f nA;ADC Voltage (V);A_{LED}", pmt, HV, LL));
   ...
   legA->Draw();
   c3Cal->SaveAs(Form("test_%d/r%d_asymTrends_auto_20cyc_Cal.png",npat,run));
   }
