...
//     ofstream outfile2("sum.txt");
     tree->Draw("qwk_mod0ch6.hw_sum_raw:CodaEventNumber","","goff");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	TCut ped_cut = Form("CodaEventNumber>10&&qwk_mod0ch6.hw_sum_raw<%f",chsum);
	TCut even_cut = Form("qwk_mod0ch6.hw_sum_raw<%f&&Entry$>10&&(Entry$/2.0-int(Entry$/2))==0",chsum);	  
	TCut odd_cut = Form("qwk_mod0ch6.hw_sum_raw<%f&&Entry$>10&&(Entry$/2.0-int(Entry$/2))!=0",chsum);	  
	tree->Draw("qwk_mod0ch6.hw_sum_raw>>hped",ped_cut,"goff");
	h1_ped = (TH1F*)gDirectory->FindObject("hped");
	h1_ped->SetDirectory(gROOT);
	tree->Draw("qwk_mod0ch6.hw_sum_raw>>hpedE",even_cut,"goff");
	h1_ped_even = (TH1F*)gDirectory->FindObject("hpedE");
	h1_ped_even->SetDirectory(gROOT);
	tree->Draw("qwk_mod0ch6.hw_sum_raw>>hpedO",odd_cut,"goff");
