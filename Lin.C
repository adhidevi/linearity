...
//root -l AnalyzeMps.C'(3899,5407,10.0,745,0.15,0.671,120,0.005,0.055,1555000,0.005,0.055,0.010,0.045,0.010,0.045,-0.002,0.002,0.020,0.040)'
void Lin(Int_t run,Int_t pmt,Double_t preAmp,Int_t HV,Double_t LL,Double_t Iout,Int_t FF,Double_t asym_X1,Double_t asym_X2,Int_t chsum_min,Int_t chsum,Double_t asym1_X1,Double_t asym1_X2,Double_t asym8_X1,Double_t asym8_X2,Double_t asymF_X1,Double_t asymF_X2,Double_t asymP_X1,Double_t asymP_X2,Double_t asym_Y1,Double_t asym_Y2){

	 TChain* asym = new TChain("asym");
	 asym->Add("./rootfiles/asymmetry.root");
	 Float_t fltr_evt,fltr_asym,sum;
     vector<Double_t>pairNumber, Filter_Event_Num, Filter_Asymmetry, pairSum;
     asym->SetBranchAddress("Filter_Event_Num",&fltr_evt);
     asym->SetBranchAddress("Filter_Asymmetry",&fltr_asym);
     asym->SetBranchAddress("pairSum",&sum);

	 int nentries = asym->GetEntries();
	 for(int i=0;i<nentries;i++){
		asym->GetEntry(i);
		Filter_Event_Num.push_back(fltr_evt);
		Filter_Asymmetry.push_back(fltr_asym);
		pairSum.push_back(sum);
	 }

     cout<<"Total data points: "<<nentries<<endl;
   ...
   for (Int_t j=0; j<nentries;j++) {
	Double_t aaa = Filter_Event_Num[j];
	Double_t bbb = Filter_Event_Num[j-1];
	Double_t ccc = Filter_Event_Num[j-1]+1;
	Double_t asymm = Filter_Asymmetry[j];
        Double_t summ = pairSum[j];
//	cout<<j<<"\t"<<asymm<<"\t"<<summ<<"\t"<<aaa<<"\t"<<bbb<<"\t"<<ccc<<endl;
	if(aaa!=ccc && aaa!=0){
	group++;
	cout<<group<<"\t"<<group_data_count<<"\t"<<j<<"\t"<<Filter_Asymmetry[j]<<"\t"<<aaa<<"\t"<<bbb<<"\t"<<ccc<<endl;
	group_data_count = 0;
	}else if(aaa!=0){
	group_data_count++;
//	cout<<group_data_count<<endl;
    if(group_data_count>50&&group_data_count<=1150){
	  h[group-1]->Fill(asymm);
	  Navg[group-1] += summ/2/1100/samples/blocks;
    }
    }
    }
