...
void AnalyzeMps_quartet(Int_t run,Int_t pmt,Double_t preAmp,Int_t HV,Double_t LL,Double_t Iout,Int_t FF,Double_t asym_X1,Double_t asym_X2,Double_t chsum,Double_t asym1_X1,Double_t asym1_X2,Double_t asym8_X1,Double_t asym8_X2,Double_t asymF_X1,Double_t asymF_X2,Double_t asymP_X1,Double_t asymP_X2,Double_t asym_Y1,Double_t asym_Y2)
{
     ...
     TH2F *h1_asym_my = new TH2F("h1_asym_my", "", nentries/20, 0, nentries/4, 800, asym_X1, asym_X2);
...
//        havg[f]->SetMaximum(8);
   ...
   for (Long64_t jentry=10; jentry<nentries;jentry++) {
	...
	if(EventNum[jentry]/2.0-int(EventNum[jentry]/2) ==0){
	  ...
	  if(EventNum[jentry]/4.0 - int(EventNum[jentry]/4) ==0){
	  Q0++;
	  raw_sum0[Q0] = hw_sum_raw[jentry]-pedAve;
	  }else if((EventNum[jentry]-1)/4.0 - int((EventNum[jentry]-1)/4) ==0){
	  Q1++;
	  raw_sum1[Q1] = hw_sum_raw[jentry]-pedAve;
	  }else if((EventNum[jentry]-2)/4.0 - int((EventNum[jentry]-2)/4) ==0){
	  Q2++;
	  raw_sum2[Q2] = hw_sum_raw[jentry]-pedAve;
	  }else if((EventNum[jentry]-3)/4.0 - int((EventNum[jentry]-3)/4) ==0){
	  Q3++;
	  raw_sum3[Q3] = hw_sum_raw[jentry]-pedAve;
	  }
	}
	
    for (int i=1; i<octsize/2; i++){
    	  EvenSum[i] = raw_sum0[i]+raw_sum2[i];
    	  ChSum_even = EvenSum[i];
    	  OddSum[i] = raw_sum1[i]+raw_sum3[i];
    	  ChSum_odd = OddSum[i];
	  sum = EvenSum[i] + OddSum[i];
	  diff = EvenSum[i] - OddSum[i];
	  outfile<<"pattern no: "<<i<<"\tsum: "<<sum<<"\t diff: "<<diff<<endl;
	  Asymmetry = fabs(diff/sum);
	  Asymmetry_Ped = (EvenSum[i] - OddSum[i])/(EvenSum[i] + OddSum[i]+4*pedAve);
	  ...
	  group_data_count++;
       if(group_data_count>10 && group_data_count<=600){
	  h[group-1]->Fill(Asymmetry);
	  Navg[group-1] += sum/4/590/samples/blocks;
       }
       }
